...
- Post-review polish pass from Espen's direct feedback, before closing: `--version` gained a `-v` shorthand; `add`/`edit`/`fetch`'s one-line summaries were tightened ("Creates a new tenant.", "Updates a tenant.", "Fetches from the given tenant ID."); a new `crmfetch show <id>` command was added (prints one tenant's full JSON, same pattern as `fetch`/`delete`'s id lookup); and a `crmfetch fetch 3`-style example was added to the id parameter's help text. Also added a banner (ASCII art naming the app) shown above `--help` output, via Cyclopts' `help_prologue` - had to set `help_format="plaintext"` app-wide since the default Markdown formatting collapsed the banner's line breaks into one line (this also changes how backticks render elsewhere in help text, so those were removed from the remaining docstrings for consistency). Two new tests added (`show`, `-v`); full suite now 43 passing.
- Two more from the same feedback pass: `--help`/`--version` are Cyclopts' own auto-registered commands with no group assigned by default, so they were showing in the same "Commands" panel as add/delete/edit/fetch/list/show, blurring flags with commands - moved them into their own "Options" panel by setting `.group` on the two auto-registered sub-apps directly. Also dropped the top-level `Usage: crmfetch COMMAND` line (`usage=""`) since the Commands panel right below it already says everything it would - subcommand usage lines (e.g. `crmfetch fetch TENANT-ID`) are untouched, since those show real argument syntax.
- Final round, after ticket 05 added `settings`: the banner now shows `v{version} - <repo url>` under the ASCII logo (computed once via `get_current_version()` at CLI startup - each invocation is a fresh process, so this is never stale). `--version` was then hidden from the Options listing too (same treatment as `--help` - stays fully functional via `--version`/`-v`, just redundant to list now that the banner shows it). Commands got an explicit display order (`add, delete, edit, fetch, show, list, settings` via `sort_key` on each) since Cyclopts sorts alphabetically by default and `show` was requested to sit right after `fetch` instead. `fetch`'s summary gained "...into its specified directory" for clarity on where fetched data lands.
